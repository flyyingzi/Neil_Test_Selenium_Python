...
# 引自：http://www.w3cschool.cc/python/python-modules.html
...
当前目录
如果不在当前目录，Python则搜索在shell变量PYTHON_PATH下的每个目录。
如果都找不到，Python会察看默认路径。UNIX下，默认路径一般为/usr/local/lib/python/

模块搜索路径存存储在system模块的sys.path变量中。变量里包含当前目录，PYTHON_PATH和由安装过程决定的默认目录。
"""


"""
# 19.5 PYTHON_PATH 变量
作为环境变量，PYTHON_PATH由装在一个列表里的许多目录组成。PYTHON_PATH的语法和shell变量PATH的一样。

在Windows系统，典型的PYTHON_PATH如下：
set PYTHON_PATH=c:\python20\lib;

在UNIX系统，典型的PYTHON_PATH如下：
